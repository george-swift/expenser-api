import pytest
from src.utils.validation import validate_user, validate_expense_payload
...
def test_validate_expense_payload_invalid_category():
    ...
    with pytest.raises(ValueError, match=f"Invalid categoryId: {data['categoryId']}"):
        validate_expense_payload(data)
...
def test_validate_expense_payload_invalid_amount():
    ...
    with pytest.raises(ValueError, match="Amount must be a positive non-zero number."):
        validate_expense_payload(data)
